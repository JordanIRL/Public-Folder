...
$ownCountsHash = @{}
$serialBuckets = @{}
$dellLaptopCountsHash = @{}

$now = Get-Date
$ageBuckets = @(0, 0, 0, 0, 0)  # 0-7, 7-14, 14-28, 28-60, 60+
...
foreach ($d in $devices) {
    ...
    # Dell laptop models (exclude thin clients, desktops, servers)
    if ($d.manufacturer -match '^Dell' -and $d.model -match '^(Latitude|XPS|Precision|Inspiron|Vostro|Dell Pro(?!\s*(Desktop|Mini|Compact|Tower|Thin)))') {
        $dellLaptopCountsHash[$d.model]++
    }
    ...
    if ($syncTime) {
        ...
        if     ($days -gt 60) { $ageBuckets[4]++ }
        elseif ($days -gt 28) { $ageBuckets[3]++ }
...
$modelCounts = Convert-ToCountList -Counts $modelCountsHash
...
$dellLaptopCounts = Convert-ToCountList -Counts $dellLaptopCountsHash
...
$checkinCounts = @(
    ...
    [PSCustomObject]@{ Name = '28 - 60 Days'; Count = $ageBuckets[3] }
    [PSCustomObject]@{ Name = '> 60 Days';    Count = $ageBuckets[4] }
)
...
try {
    ...
    $dellLaptopCounts  = Limit-TableItems -Items $dellLaptopCounts  -MaxSlots $maxSlots
    ...
    $rOs  = Add-Table -Title "Operating Systems"  -Col1 "L" -Col2 "N" -ColEnd "O" -Items $osCounts    -Accent $tableAccent -StartRow 12 @tableArgs
    $rOwn = Add-Table -Title "Device Ownership"  -Col1 "Q" -Col2 "S" -ColEnd "T" -Items $ownCounts   -Accent $tableAccent -StartRow 12 @tableArgs

    $nextRowOs = $rOs + 2
    $rDellLaptops = Add-Table -Title "Dell Laptop Models" -Col1 "L" -Col2 "N" -ColEnd "O" -Items $dellLaptopCounts -Accent $tableAccent -StartRow $nextRowOs @tableArgs
    ...
    $smX306B = @($devices | Where-Object { $_.model -eq 'SM-X306B' })
    if (-not $smX306B) { $smX306B = $naPlaceholder }
    $smX306B | Export-Excel @exportCommon -WorksheetName 'SM-X306B' -TableName 'SMX306B' | Out-Null
    ...
    @(
        ...
        @{ Name = 'Android Fully Managed';      Table = 'AndroidFullyManaged';    Filter = { $_.operatingSystem -match '^Android' -and $_.deviceEnrollmentType -in @('androidEnterpriseFullyManaged', 'androidEnterpriseDedicatedDevice') } }
        @{ Name = 'Android Corp Work Profile';  Table = 'AndroidCorpWorkProfile'; Filter = { $_.operatingSystem -match '^Android' -and $_.deviceEnrollmentType -eq 'androidEnterpriseCorporateWorkProfile' } }
    ) | ForEach-Object {
    ...
    $sheetColors = @{
        ...
        'SM-X306B'                  = [System.Drawing.Color]::FromArgb(196, 181, 253)
    }
